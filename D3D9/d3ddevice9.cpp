...
HRESULT __stdcall NewIDirect3DDevice9::DrawIndexedPrimitive(D3DPRIMITIVETYPE  Type,INT BaseVertexIndex,UINT MinVertexIndex,UINT NumVertices,UINT startIndex,UINT primCount) 
{
...
HRESULT __stdcall NewIDirect3DDevice9::SetStreamSource(UINT StreamNumber,IDirect3DVertexBuffer9* pStreamData,UINT OffsetInBytes,UINT Stride) 
{
	...
	return m_pd3dDevice->SetStreamSource( 
		StreamNumber, 
		((NewIDirect3DVertexBuffer9*)pStreamData)->m_pd3dVertexBuf, 
		OffsetInBytes, 
		Stride); 
}
...
HRESULT NewIDirect3DDevice9::GetDevice(IDirect3DDevice9** ppDevice)
{
	if(ppDevice == NULL)
	{
		return D3DERR_INVALIDCALL;
